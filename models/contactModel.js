...
const createContact = async (email, phone, linkedId = null, linkPrecedence = "primary") => {
  ...
  const [rows] = await db.query(`SELECT * FROM Contact WHERE id = ?`, [result.insertId]);
  return rows[0];
};
...
const updateContact = async (id, data) => {
  ...
  await db.query(`UPDATE Contact SET ${setClause}, updatedAt = NOW() WHERE id = ?`, [...values, id]);
};
